# This file is from the "polyomino-enumerator" repo at
# https://github.com/mggg/polyomino-enumerator
# however, the output has been modified to print to 
# stdout in a jsonl format for easier parsing using 
# the binary-ensemble cli tool.
#
# Original Author: Zach Schutzman
# Modified by: Peter Rock
...
function enumerator(grid, om_sizes, num_parts, cont="rc", io=false)
    ...
    if io
        ...
        gr = grid
        np = num_parts
        # outfile = open(replace("./enumerations/enum_$(gr)_$(os)_$(Int8(np))_$(cont).jsonl"," "=>""),"w")
    end
    ...
    function recurs_part(plan,steps)
        if steps == 0
            if sum(sum( [oms[j] for j in plan])) == grid[1]*grid[2] 
                count +=1
                if io
                    ...
                    strrep = replace(strrep,"]"=>"")
                    strrep = replace(strrep, "\n" => "")
                    strrep = "{\"assignment\":[" * strrep * "],\"sample\":$count}\n"
    ...
    print(Base.stderr, "NEED TO DO ",length(fd[(1,1)]), " TOP-LEVELS\n")
...
print(Base.stderr, "\n $a:$a: \n",(enumerator([a, b],[c], d,"rc",true)[1]))
print(Base.stderr,"\n")
print(Base.stderr,"took $(canonicalize(Dates.CompoundPeriod(now()-tic))) \n")
